...
## <b id="6"> ✔️ 주요 기능 </b>
 - ### 회원가입 / 유효성 검사 / 로그인 / 수정 / 탈퇴
   - 닉네임, 아이디, 비밀번호, 프로필 이미지로 구성했고 **글자 수 제한, 문자열 내 빈 공백 여부**를 확인합니다.
   - 현재 회원일 경우 **정보 수정/회원 탈퇴**를 할 수 있습니다.
  ...
  - ### 마커/정보창 생성, 현재 위치 조회/이동, 주소 변환, 주소 검색/이동
    - 현재 위치를 **마커로 생성**했고 마커를 클릭하면 클릭한 좌표를 **주소로 변환해서 정보창에 출력**하도록 했습니다.
    - 주소를 검색하면 해당 **주소를 좌표로 변환하여 이동**합니다.

      <img src="https://github.com/user-attachments/assets/6161be0b-0924-413b-b4bf-18775b67a19b" width="50%"/>   
     
 - ### 가게 등록 / 수정 / 삭제 / 리스트
    - **등록**은 **가게 관련된 정보와 네이버 지도의 좌표로 구성**하여 **유효성 검사를 적용**하였고 **다중 파일 업로드**가 가능합니다.
    - **수정**은 **연관 편의 메서드 방식으로 수정**하였고 **실제 파일은 기존 파일을 삭제하고 새로 받은 파일로 재 생성**합니다.
    - **삭제**는 **DB 삭제뿐만이 아닌 실제 저장한 파일도 찾아 삭제 처리**합니다.
    - **리스트**는 **회원이 등록한 가게들을** **페이징 처리하여** 리스트로 담았습니다.
...
🔗 [깃허브 브랜치 전략 - 브랜치 세팅하기](https://blog.naver.com/kyk7777_/223402809943)
